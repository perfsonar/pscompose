  ...
  htmx.defineExtension('client-side-templates', {
    transformResponse: function(text, xhr, elt) {
      ...
      if(nunjucks){
        ...
        let event = new CustomEvent('htmx:nunjucks-environment', {
            detail: {
      ...
      if (nunjucksTemplate) {
        ...
        if(elt.getAttribute("hx-context")){
          try { 
            var extra = JSON.parse(elt.getAttribute("hx-context"));
            for(let k in extra){
              data[k] = extra[k];
            }
          } catch(e) {
            console.error(`Error parsing JSON from hx-context: '${elt.getAttribute("hx-context")}'`);
            console.error(e)
          }
        }
        if (template) {
          return nunjucksEnvironment.renderString(template.innerHTML, data)
        } else {
